// -------------------------------------------------------------------------- //
// Import original Excel file `$wtid_data/Database.xls` to Stata,
// if not done already.
// -------------------------------------------------------------------------- //


assert fileexists("$wtid_data/Database.xls")
if fileexists("$work_data/original-wtid-db.dta") {
	quietly ashell date -r $work_data/original-wtid-db.dta +%s
	local imported = r(o1)
	quietly ashell date -r $wtid_data/Database.xls +%s
	local updated_db = r(o1)
	local cmd = "find $wtid_data/csv/*.csv -type f -printf '%T+\n' | sort | tail -1"
	quietly ashell `cmd'
	quietly ashell date -d "$(echo `r(o1)' | tr "+" " ")" +%s
	local updated_files = r(o1)
	if (`imported' > `updated_db' & `imported' > `updated_files') {
		di "No excel-to-Stata needed!"
		exit
	} 
	else {
		di "Converting excel-to-Stata."
	}
}
